...
一覧の取得には数秒かかる（実測 102 件で約 8 秒）ので、セッションごとに覚えます。スキルやプロジェクトのコマンドを足したら `C-u C-c C-s` で取り直してください。取得はペインの入力欄を実際に打って行うため、**入力欄に書きかけのテキストがあるときは何もしません**（下書きを消さないため）。`/model` や `/config` のように送信後に対話画面が出るコマンドは、送るところまでを担当します。画面の操作はキー透過モード（`C-c C-t`）でどうぞ。
