...
`[New]` を選ぶと新規セッションを作成します。作業ディレクトリを聞かれ、**ディレクトリ名 + 2 桁の連番**がセッション名になります（`myproj00`）。同じディレクトリでもう 1 体立ち上げると `myproj01`、`myproj02` …と増えます（`C-c C-a` → `+`。下記「エージェントを増やす」）。zellij セッションは **detached（バックグラウンド）** で作られ、`zellij-send-default-command`（デフォルト: `claude`）が新しいペインで起動します。ペイン ID を記憶して送受信するため、ターミナルでの attach は不要です。途中経過を生で見たいときは任意のターミナルで `zellij attach セッション名` してください。
...
バッファにテキストを書いて `C-c C-c` で送信します。送信成功後、送信開始から編集されていなければバッファをクリアします。送信中に編集した場合は、送信済み本文を含め内容全体を残します（返信バッファも閉じません）。
...
| `+`  | エージェントを増やす（同じディレクトリにもう 1 体）                    |
| `q`  | セッションを削除（zellij セッションを消してバッファを閉じる。確認あり） |

### エージェントを増やす

`C-c C-a` → `+`（`M-x zellij-send-add-agent`）で、**いまのバッファと同じディレクトリ**にもう 1 体エージェントを立ち上げます。作業ディレクトリは聞かれません。

セッション名はいまのセッションの基底名に空いている連番を付けたもの（`myproj00` から呼べば `myproj01`）。番号は zellij のセッション一覧と、開いている黒板バッファの両方を見て決めるので、作成直後で一覧にまだ出ていないセッションとも衝突しません。途中の番号が空いていればそこを埋めます。

**エージェント 1 体につき zellij セッション 1 つ**なので、送信・自動受信・`q`（終了）はこれまでどおりバッファ単位で効きます。1 体だけ終了しても他は残ります。ダッシュボード（`d`）には全員が別行で並びます。

`a`（transcript 表示）は初回に対象の JSONL を選び、その対応付けを各セッションのバッファに保持します。更新日時から別エージェントの履歴を自動選択しません。
...
選択肢の検出は claude の `❯ 1.`、codex の `› 1.`、agy の `> 1.` を拾います。**ASCII の `>` は agy のバッファでだけ有効**です（`zellij-send-prompt-marker-alist`）。全 CLI で有効にすると本文中の markdown 引用（`> 1. …`）を選択肢と誤検出し、ダッシュボードの数字キーが「選択のつもりのただの入力」を送ってしまうためです。agy のバッファ内での誤検出は残ります。
...
引数を取るコマンドは確定後に引数も尋ねます。Claude Code の引数の出し方は 2 通りあり、どちらも扱えます。

- **ヒント型**（`/effort  [low|medium|high|…]`、`/add-dir  <path>`）: 選択肢が決まっているものは補完候補になり、`<path>` や `<dir>` の類は Emacs のディレクトリ補完になります。それ以外は自由入力です
- **メニュー型**（`/config`・`/plugin`・`/claude-api` など、ペイン側でカーソルで選ぶタイプ）: ペインが出す候補を集めてミニバッファで選ばせます。`/plugin` → `enable` → プラグイン名のように**入れ子**になっていても、1 つ選ぶたびに次の候補を聞き直します。`/config` の `autoScroll=` → `autoScroll=true` も同じ仕組みです

メニューの候補は Claude Code 側が**一度に 12 件までしか出しません**。候補に無い文字列を打って `RET` すると、それで絞り込んだ候補を出し直すので、数百あるプラグイン名も `install` → `air` → `airwallex-…` の順で辿れます。引数を取らないコマンドはそのまま送ります。
...
初回は Claude の session ID に対応する名前の JSONL を選んでください。候補が一つでも確認します。
対応付けはバッファを閉じるまで保持します。Claude 側で会話を切り替えた場合などは
`M-x zellij-send-select-transcript` で選び直してください。選択済みのファイルが消えても、
別の履歴に自動で切り替わることはありません。
...
既定（`zellij-send-askq-auto` = `keys`）では、質問が出てもミニバッファは開きません。黒板バッファを表示して[キー透過モード](#キー透過モードc-c-c-t)に自動で入るので、選択肢の説明や事例を画面で読みながら数字キー（`↑↓` / `RET` も可）で答えられます。質問が消えると自動で解除します（自分で `C-c C-t` を押して入ったモードは勝手に切りません）。ウィンドウは表示するだけで選択はしないので、打つ前にそのウィンドウへ移ってください。

ラベルだけをミニバッファで選ぶ旧挙動にしたければ `minibuffer`、自分のタイミングで開きたければ `nil` にして `C-c C-q`（または `C-c C-a` → `u`）を使ってください。途中で `C-g` を押せば、ペインの質問はそのままで抜けられます。
